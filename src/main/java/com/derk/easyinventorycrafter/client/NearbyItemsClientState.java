...
public final class NearbyItemsClientState {
	...
	public static void requestUpdate() {
		if (ClientPlayNetworking.canSend(RequestNearbyItemsPayload.ID)) {
			...
			ClientPlayNetworking.send(RequestNearbyItemsPayload.ID, buf);
		}
	}

	public static void applyPayload(NearbyItemsPayload payload) {
		entries = payload.entries() == null ? Collections.emptyList() : List.copyOf(payload.entries());
		recipeFinderStacks = payload.recipeFinderStacks() == null ? Collections.emptyList() : List.copyOf(payload.recipeFinderStacks());
	...
	private static void requestHighlight(ItemStack stack, boolean aimAfterResponse) {
		...
		if (ClientPlayNetworking.canSend(NearbyHighlightRequestPayload.ID)) {
			NearbyHighlightRequestPayload payload = new NearbyHighlightRequestPayload(stack.copyWithCount(1));
			PacketByteBuf buf = PacketByteBufs.create();
			payload.encode(buf);
			ClientPlayNetworking.send(NearbyHighlightRequestPayload.ID, buf);
		}
	}
	...
	public static void setHighlight(List<BlockPos> positions, int ticks) {
		if (positions == null || positions.isEmpty()) {
			...
			List<BlockPos> normalized = new ArrayList<>(positions.size());
			for (BlockPos pos : positions) {
				if (pos != null) {
					normalized.add(pos.toImmutable());
				}
			}
			highlightPositions = normalized.isEmpty() ? Collections.emptyList() : List.copyOf(normalized);
		}
