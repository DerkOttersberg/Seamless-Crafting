...
public class EasyInventoryCrafterClient implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		ClientPlayNetworking.registerGlobalReceiver(NearbyItemsPayload.ID, (client, handler, buf, responseSender) -> {
			NearbyItemsPayload payload = NearbyItemsPayload.decode(buf);
			client.execute(() -> NearbyItemsClientState.applyPayload(payload));
		});

		ClientPlayNetworking.registerGlobalReceiver(NearbyHighlightResponsePayload.ID, (client, handler, buf, responseSender) -> {
			NearbyHighlightResponsePayload payload = NearbyHighlightResponsePayload.decode(buf);
			client.execute(() -> NearbyItemsClientState.setHighlight(payload.positions(), EasyInventoryCrafterConfig.getHighlightDurationTicks()));
		});
		...
		WorldRenderEvents.LAST.register(context -> {
			...
			net.minecraft.client.render.VertexConsumer consumer = consumers.getBuffer(RenderLayer.getLightning());

			List<Box> labelBoxes = new ArrayList<>();
