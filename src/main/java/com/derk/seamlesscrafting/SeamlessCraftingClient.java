...
import com.derk.seamlesscrafting.client.SeamlessHighlightRenderer;
...
public class SeamlessCraftingClient implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		ClientPlayNetworking.registerGlobalReceiver(NearbyHighlightResponsePayload.ID, (payload, context) -> {
			NearbyItemsClientState.setHighlight(payload.positions(), SeamlessCraftingConfig.getHighlightDurationTicks());
		});
		...
		ClientTickEvents.END_CLIENT_TICK.register(new ClientTickEvents.EndTick() {
			...
			public void onEndTick(MinecraftClient client) {
				...
				tickCounter++;
				if (tickCounter >= SeamlessCraftingConfig.getAutoRefreshTicks()) {
		...
		WorldRenderEvents.BEFORE_TRANSLUCENT.register(context -> {
			...
			for (BlockPos pos : NearbyItemsClientState.getHighlightPositions()) {
				if (!renderedPositions.add(pos)) {
					continue;
				}

				Box fullBox = renderHighlightedBlock(client, context.consumers(), matrices, cameraPos, pos, alpha);
				if (fullBox == null) {
					continue;
				}
				...
				if (partnerPos != null) {
					renderedPositions.add(partnerPos);
					Box partnerBox = renderHighlightedBlock(client, context.consumers(), matrices, cameraPos, partnerPos, alpha);
					if (partnerBox != null) {
						fullBox = combineBoxes(fullBox, partnerBox);
					}
				}

				if (SeamlessCraftingConfig.isDistanceLabelEnabled()) {
					SeamlessHighlightRenderer.renderDistanceLabel(
						client,
						matrices,
						context.consumers(),
						cameraPos,
	...
	private static @Nullable Box renderHighlightedBlock(
		...
		for (Box shapeBox : shape.getBoundingBoxes()) {
			...
			var consumer = consumers.getBuffer(RenderLayers.lightning());
			SeamlessHighlightRenderer.renderBox(matrices, consumer, cameraPos, box, alpha);
		}
